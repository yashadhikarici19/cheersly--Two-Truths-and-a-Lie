...
slackEvents.on("message", (evt, body) => {
  try {
    if (evt.subtype === "channel_join") {
      ...
      const game = (games[evt.channel][evt.user] = new Game(
        web,
        evt.user,
        evt.channel
      ));
...
slackInteractions.action(
  { within: "dialog", callbackId: "accept" },
  (payload, respond) => {
    try {
...
slackInteractions.action(
  { within: "dialog", callbackId: "guess" },
  (payload, respond) => {
    try {
...
app.get("/localhost.mlh.io/activities/build-slack-apps", (req, res) =>
  res.redirect("http://localhost.mlh.io")
);
